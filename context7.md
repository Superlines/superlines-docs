...
curl -H "Authorization: Bearer sl_live_YOUR_API_KEY" \
  "https://analytics.superlines.io/api/analytics/metrics?domainId=abc123"
```
...
Core metrics for a brand.

Parameters: `domainId` (required), `startDate`, `endDate`, `granularity` (total/daily/weekly/monthly), `metrics` (comma-separated: brand_visibility, citation_rate, mentions, share_of_voice), `groupBy` (llm_service, country, topic).
...
```json
{
  ...
  "data": {
    "metrics": {
      "brand_visibility": { "value": 32.5, "change": 2.1, "trend": "up" },
      "citation_rate": { "value": 18.7, "change": -0.5, "trend": "down" },
      "share_of_voice": { "value": 22.3, "change": 1.8, "trend": "up" }
    }
  }
}
```
...
Citation data at domain or URL level. Parameters: `domainId` (required), `aggregateBy` (domain/url), `startDate`, `endDate`, `limit`.

#### GET /analytics/brand-mentions

Brand mention data with sentiment. Parameters: `domainId` (required), `startDate`, `endDate`, `groupBy` (brand, llm_service, country).

#### GET /analytics/prompts

Tracked prompts for a brand. Parameters: `domainId` (required), `status` (active/paused/all), `label`, `limit`, `offset`.

#### GET /analytics/weekly-performance

Week-over-week trends. Parameters: `domainId` (required), `weeksBack` (default: 4), `metrics`.

#### GET /analytics/competitor-insights

Competitor ranking data. Parameters: `domainId` (required), `startDate`, `endDate`, `topN` (default: 10).

#### GET /analytics/content-opportunities

Topics with high volume but low brand visibility. Parameters: `domainId` (required), `minMentions` (default: 3).

#### GET /analytics/dashboard-insights

High-level summary. Parameters: `domainId` (required).
...
Success: `{ "success": true, "data": { ... }, "meta": { "total": 100, "limit": 10, "offset": 0 } }`

Error: `{ "success": false, "error": { "code": "UNAUTHORIZED", "message": "..." } }`
...
- **`get_citation_data`** — Citations at domain or URL level. Parameters: `brands` (array), `aggregateBy` (domain/url), `limit`.
- **`analyze_brand_mentions`** — Mention counts, sentiment, SOV. Parameters: `brands` (array), `groupBy` (array), `minMentions`.
- **`get_query_data`** — Search queries with intent, user journey stage, brand mentions. Parameters: `brands` (array), `filters`, `sortBy`.
...
- **`webpage_crawl`** — Fetch and parse any webpage. Returns metadata, structured data, HTML structure, text. Parameter: `url` (required, must include `https://`).
- **`webpage_audit`** — Full LLM-friendliness audit: technical SEO, content quality, metadata, schemas, writing quality. Parameters: `url` (required), `includeCodeSnippets`.
...
- **`get_fanout_query_insights`** — Web searches LLMs perform when answering user questions.
- **`get_competitive_gap`** — Prompts where competitors lead or where you're winning. Includes visibility comparison and AI recommendations.
- **`get_top_cited_url_per_prompt`** — The #1 cited URL for each tracked prompt.
- **`get_best_performing_prompt`** — Best prompts by visibility or citation rate. Parameter: `metric` (visibility/citations/both).
- **`get_period_comparison`** — Current period vs. previous period. Parameter: `periodDays` (default: 30).
- **`analyze_competitor_url`** — Deep-dive competitor URL analysis with recommendations. Parameters: `url` (required), `userBrand`, `targetPrompt`.

#### Advanced Analytics

- **`get_competitor_insights`** — Top cited domains and most mentioned brands.
- **`analyze_sentiment`** — Sentiment breakdown (positive/neutral/negative). Parameter: `groupBy` (llm_service/topic/country).
- **`find_content_opportunities`** — High-volume topics with low brand visibility.
- **`get_analytics_summary`** — High-level summary with overall metrics and top topics.

#### Strategic Tools

- **`generate_strategic_action_plan`** — Priority-ranked action plan with improvement scores. Parameters: `brands` (array), `focusArea` (all/visibility/citations/sentiment/content_gaps), `maxRecommendations` (default: 5, max: 10).
- **`get_next_actions`** — Top 3 most impactful actions to take next.
...
- **`website_audit`** — Technical SEO audit with health score (0–100). Parameter: `url` (required).
- **`keyword_research`** — Search volume, difficulty, CPC, intent, related keywords. Parameters: `keyword` (required), `country` (default: us), `language` (default: en).
